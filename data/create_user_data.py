...
if __name__ == '__main__':
    orders = pd.read_csv('content/data/raw/orders.csv')
    ...
    products = pd.read_csv('/content/data/raw/products.csv')
    ...
    if not os.path.isdir('/content/data/processed'):
        os.makedirs('/content/data/processed')

    user_data = df.groupby('user_id', sort=False).apply(parse_user).reset_index()
    user_data.to_csv('/content/data/processed/user_data.csv', index=False)
